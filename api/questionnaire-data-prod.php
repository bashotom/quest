 ...
 * Compatible with MySQL 8.0+ and MariaDB 10.11+
 * 
 * Endpoints:
...
try {
    $pdo = new PDO(
        ...
        [
            ...
            // MariaDB/MySQL compatibility: Ensure UTF8MB4 strict mode
...
function saveQuestionnaireAnswers($pdo) {
    try {
        ...
        // Upsert questionnaire answers (MariaDB/MySQL compatible)
        $stmt = $pdo->prepare("
...
function loadQuestionnaireAnswers($pdo) {
    try {
        ...
        // MariaDB/MySQL compatible query with computed column
        $stmt = $pdo->prepare("
...
function validateAnswers($answers) {
    ...
    foreach ($answers as $questionId => $value) {
        ...
        // MariaDB/MySQL: Allow broader range including 0 for "trifft nicht zu"
 ...
 * MariaDB/MySQL compatible cleanup using computed expires_at column
 */
...
/**
 * Database compatibility check (useful for debugging)
 */
function checkDatabaseCompatibility($pdo) {
    try {
        // Check if we're running MariaDB or MySQL
        ...
        $stmt = $pdo->query("SELECT JSON_VALID('{\"test\": 123}') as json_valid");
        $jsonSupport = $stmt->fetchColumn();
        
        return [
            'database' => $isMariaDB ? 'MariaDB' : 'MySQL',
            'version' => $version,
            'json_support' => (bool)$jsonSupport
        ];
    } catch (Exception $e) {
